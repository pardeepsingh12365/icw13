...
exports.run = async (bot, message, args, command, db) => {
  ...
  if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
    ...
    for (const video of Object.values(videos)) {
      ...
      await bot.addSong(message, video, voiceChannel, true);
    }
  } else {
    setTimeout(function() {
...
\`\`you can turn on/off song selection with command ${config.prefix}songselect\`\``)
          ...
          //console.log(video3)
          await bot.addSong(message, video3, voiceChannel, false);
        } else {
          const video3 = await videos[0];
          //console.log(video3)
          await bot.addSong(message, videos[0], voiceChannel, false);
        }
      });
    }, 200);
  }
